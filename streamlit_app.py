...
# --- PAGE CONFIG ---
st.set_page_config(page_title="Savant v31: Universal Parser", layout="wide")
st.title("🏀 Savant v31: The Universal Parser")
...
# --- STEP 2: DEEP STATS (UNIVERSAL PARSER) ---
        ...
        if 'boxscore' in res and 'teams' in res['boxscore']:
            for team in res['boxscore']['teams']:
                for stat in team.get('statistics', []):
                    # We check BOTH the 'name' (internal key) and 'label' (display name)
                    ...
                    try:
                        ...
                        if "-" in raw:
                            val = float(raw.split('-')[1]) # Attempts
                        else:
                            val = float(raw)
                    except: val = 0.0
                    ...
                    # 4. FIELD GOALS (Attempts) - EXCLUDE 3PT
                    # We want "Field Goals" or "FG", but NOT "3PT" or "Three Point"
                    is_fg = ("field" in nm or "fg" in lbl)
                    is_3pt = ("three" in nm or "3pt" in lbl)
                    if is_fg and not is_3pt:
                        data['fga'] += val
                        
                    # 5. FREE THROWS (Attempts)
...
# --- STEP 3: EXECUTION ---
with st.spinner("Parsing Live Metrics..."):
...
if not live_games:
    ...
    for i, game in enumerate(live_games):
        ...
        if mins > 2.0:
            ...
            # SAFEGUARD: If we didn't find FGA, return 0 Pace (not negative)
            if deep['fga'] > 0:
