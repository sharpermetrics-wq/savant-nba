    ...
    for i, game in enumerate(live_games):
        ...
        if mins > 2.0:
            ...
            for bet in st.session_state.bet_slip:
                if str(bet['ID']) == str(game['id']): is_tracked = True; break
            
            results.append({
                "Add": is_tracked,
                "ID": str(game['id']), # Force String
...
with st.sidebar:
    ...
    if st.session_state.bet_slip:
        ...
        for bet in st.session_state.bet_slip:
            live_data = live_game_map.get(str(bet['ID']), {"Score": "Final", "Time": "End", "Proj": 0})
